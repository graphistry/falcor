...
describe('an old reference over a newer reference', function() {
    ...
    it('should correctly assign f_abs_path after setting past references', function() {
        ...
        expect(cache.genrelist[0][f_abs_path]).to.deep.equal(['genrelist', 0]);
        expect(cache.genrelist[1][f_abs_path]).to.deep.equal(['genrelist', 1]);
    });
});
