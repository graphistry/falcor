...
module.exports = function get(walk, isJSONG) {
    return function innerGet(model, paths, seed) {
        ...
        var branchSelector = model._root.branchSelector;

        if (branchSelector && !isJSONG) {
            valueNode.json = (!model._allowFromWhenceYouCame) ?
                branchSelector(currentCachePosition) :
                branchSelector(currentCachePosition, referenceContainer);
        }

        for (i = 0, len = paths.length; i < len; i++) {
            walk(model, cache, currentCachePosition, paths[i], 0,
                 valueNode, results, derefInfo, requestedPath, optimizedPath,
                 optimizedLength, isJSONG, false, referenceContainer, branchSelector);
        }
